/* blk_file.cpp;
 implementation of class BlockFile2 and CachedBlockFile
...
void BlockFile2::fwrite_number(int value){
  put_bytes((char *) &value, sizeof(int));
}

//----------------------------------------------------------------

int BlockFile2::fread_number(){
  char ca[sizeof(int)];
...
BlockFile2::BlockFile2(char* name,int b_length){
  char *buffer;
  int l;
   
  //printf("BlockFile2 Version 1.0\n");
  ...
  if((fp=fopen(name,"rb+"))!=0){
    ...
    if (blocklength < BFHEAD_LENGTH)
      error("BlockFile2::BlockFile2: Blocks zu kurz\n",TRUE);
    
    fp=fopen(filename,"wb+");
    if (fp == NULL)
      error("BlockFile2::new_file: Schreibfehler",TRUE);
...
void BlockFile2::read_header(char* buffer)
...
void BlockFile2::set_header(char* header)
{
   fseek(fp,BFHEAD_LENGTH,SEEK_SET);
...
bool BlockFile2::read_block(Block b,int pos)
{
...
bool BlockFile2::write_block(Block block, int pos)
...
int BlockFile2::append_block(Block block)
{
   fseek(fp,0,SEEK_END);
   put_bytes(block,blocklength);
   number++;
...
bool BlockFile2::delete_last_blocks(int num)
{
   if (num>number)
      return FALSE;
		 ...
		 if (ptr == tmp)	//failed to find a free block
		 {
			 ...
			 if (dirty_indicator [ptr] == true)
			   /*line added by tao yufei*/
    			BlockFile2::write_block(cache[ptr],cache_cont[ptr]-1);
...
bool CachedBlockFile::read_block(Block block, int index)
{
	...
	if(index <= get_num_of_blocks() && index>0)
	{
		...
		{     
			...
			if (c_ind >= 0)
			{
				BlockFile2::read_block(cache[c_ind], index - 1); // ext. Num.
				...
				BlockFile2::read_block(block, index - 1); // read-through (ext.Num.) without caching
		}
		return TRUE;
	}
	else 
	...
	if(index <= get_num_of_blocks() && index > 0)
	{
		...
		else		
		{
			...
			if (c_ind >= 0)
			{
				...
				BlockFile2::write_block(block, index - 1);  // write-through (ext.Num.)
		}
		return TRUE;
	}
	else
...
// schreibt den  Cache auf Platte, gibt ihn nicht frei
{
	...
	for (i = 0; i < cachesize; i++)
		if (fuf_cont[i] != free && dirty_indicator[i])	
			BlockFile2::write_block(cache[i], cache_cont[i] - 1); // ext.Num.
}
