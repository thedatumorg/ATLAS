...
int main(int nargs, char **args)
{
	...
	while (cnt < nargs && !failed) {
		...
		else if (strcmp(args[cnt], "-ring") == 0) {
			n_ring = atoi(args[++cnt]);
			printf("ring = %d\n", n_ring);
			if (n_ring <= 0) {
				failed = true;
				break;
		...
		else if (strcmp(args[cnt], "-topK") == 0) {
			topK = atoi(args[++cnt]);
			printf("topK = %d\n", topK);
			if (topK <= 0) {
				failed = true;
