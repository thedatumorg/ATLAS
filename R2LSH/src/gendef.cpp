...
float* overlapRect(int dimension, float *r1, float *r2)
{
	    ...
	    overlap[2*i] = max_(r1[i*2], r2[i*2]);
            overlap[2*i+1] = min_(r1[i*2+1], r2[i*2+1]);
        }
...
// enlarge r to tightly contain s
{
    ...
    *mbr = new float[2*dimension];
    ...
    {
	(*mbr)[i]   = min_(r1[i],   r2[i]);

	(*mbr)[i+1] = max_(r1[i+1], r2[i+1]);
    }
}
...
float l2_dist_int(float *_p1, float *_p2, int _dim) {
//    long long p = 0;
//    long long q = 0;
//    long long pq = 0;
//    for (int i = 0; i < _dim; ++i){
